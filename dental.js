...
document.addEventListener('DOMContentLoaded', () => {
  ...
  nameInput.addEventListener('input', async () => {
    ...
    if (name) {
      const snapshot = await get(child(ref(database), 'dental'));
      if (snapshot.exists()) {
...
const savePatientData = (name, dob, parents, residence, payment, sex, nok) => {
  ...
  const newPatientRef = ref(database, `dental/${patientId}`);
...
const patientsRef2 = query(ref(database, 'dental'), limitToFirst(5000)); // Adjust the limit as needed
...
function displayVisitsPopup(patientName) {
  ...
  const visitsRef = ref(database, `dental/${patientName}/visits`);
...
const patientHistoryRef = ref(database, `dental/${patientName}/testsTaken`);
...
onValue(patientHistoryRef, (snapshot) => {
  ...
  if (snapshot.exists()) {
    ...
    records.forEach((recordObj) => {
      ...
      const testResultRef = ref(database, `dental/${patientName}/testsTaken/${recordKey}/resultsObtained`);
...
function saveConsumablesAndSundriesData(recordKey, consumablesPrice, sundriesPrice) {
  ...
  const testRecordRef = ref(database, `dental/${patient.patientId}/testsTaken/${recordKey}`);
...
editButton.addEventListener('click', function() {
  ...
  if (enteredPassword === predefinedPassword) {
    ...
    if (newDiagnosis !== null) {
      ...
      const additionalNotesRef = ref(database, `dental/${patient.patientId}/testsTaken/${recordKey}/results/additionalNotes`);
...
saveButton.addEventListener('click', async () => {
    ...
    try {
        ...
        const medicationRef = ref(database, `dental/${patient.patientId}/testsTaken/${recordKey}/results/medication/${medicationKey}`);
...
function deleteRecord(recordKey) {
  ...
  if (confirmation) {
    ...
    if (password === 'sanyu44') { // Replace 'your_password' with the actual password
      ...
      const recordRef = ref(database, `dental/${patientName}/testsTaken/${recordKey}`);
...
prescriptionInput.id = 'prescriptionInput';
  ...
  submitMedicationButton.addEventListener('click', () => {
    ...
    const patientRef = ref(database, `dental/${currentPatientName}/testsTaken/${recordKey}/results/medication`);
...
addRecordForm.addEventListener('submit', function (e) {
  ...
  const patientRef = ref(database, `dental/${patientName}`);
...
function saveAllData() {
  ...
  const patientRef = ref(database, `dental/${currentPatientName}/testsTaken/${recordKey}/results`);
