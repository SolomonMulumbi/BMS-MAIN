...
get(dateExpensesRef)
  .then((snapshot) => {
    ...
    if (dateExpensesData) {
      ...
      amountHeader.textContent = 'Reason';
      const reasonHeader = document.createElement('th');
      reasonHeader.textContent = 'Amount (UGX)';
      ...
      let totalAmount = 0;
       ...
       for (const key in dateExpensesData) {
        ...
        reasonCell.textContent = expenseData.reason;
...
function saveExpenseDetails(expense, amount, reason) {
...
saveExpenseButton.addEventListener('click', function () {
  // Retrieve expense type, amount, and reason from the input fields
  ...
  // Validate the inputs
  if (expense && !isNaN(amount) && amount > 0 && reason) {
    // Save the expense details to Firebase
    saveExpenseDetails(expense, amount, reason);
...
// Function to determine the shift based on timestamp
...
function displaySummarizedEarnings(earnings) {
  ...
  // Loop through earnings data and create table rows
  ...
  earningsChartInstance = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        data: earningsData,
        backgroundColor: colors.slice(0, earningsData.length),
        borderColor: colors.slice(0, earningsData.length).map(color => color.replace('0.2', '1')),
        borderWidth: 1
      }]
    },
    options: {
...
document.getElementById('filterButton5').addEventListener('click', () => {
  const startDate5 = new Date(document.getElementById('startDate5').value);
  const endDate5 = new Date(document.getElementById('endDate5').value);
  // Set endDate to end of the day
  endDate5.setHours(23, 59, 59, 999);

  // Call the function to display sales receipts data
  displaySalesReceipts(startDate5, endDate5);
  showMessage('Filter Completed Successfully!')
});

// Event listener for the refresh button
document.getElementById('refreshButton5').addEventListener('click', () => {
  // Clear date inputs
  document.getElementById('startDate5').value = '';
  document.getElementById('endDate5').value = '';

  // Call the function to display sales receipts data with no filter
  displaySalesReceipts(new Date(0), new Date());
  showMessage('Refreshed Successfully!')

});

// Call the function to display sales receipts data initially with no filter
displaySalesReceipts(new Date(0), new Date());
...
// Assuming currentPageSender is defined elsewhere in your code
 const currentPageSender = 'Management'; // Example
...
function sendMessage() {
  ...
  if (messageText !== '') {
    const sender = 'Management'; // You can replace 'User' with the actual username or user ID
